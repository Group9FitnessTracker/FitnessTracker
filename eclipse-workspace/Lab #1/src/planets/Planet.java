...
public class Planet 
{
	...
	public static float getLargest(Planet[] planets) {
		float max = planets[0].getMass();
		for(int i=1;i<planets.length;i++) {
			if (max<planets[i].getMass()) max = planets[i].getMass();
		}
		return max;
	}
	...
	public static void main(String[] args) {
		...
		mercury.setMass(0.44f* MASS_UNIT); //test
		System.out.println(mercury.getName());
		System.out.println(mercury.toString());//test
		System.out.println(getLargest(getAll())); //test
	}
